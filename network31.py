...
def backward(Y, Z, W, b, u, u_t, p, p_t, beta, beta_t, v, h, mean_c, c, t_input):
    for i in range(n_layers-1, 0, -1):
        if i == n_layers-1:
            ...
            if i == n_layers-2:
                u[i]   = Y[i].mm(beta[i+1]*output_burst_prob*relu_deriv(beta[i+1]))-c[i] + feedback_bias
                u_t[i] = Y[i].mm(beta_t[i+1]*output_burst_prob*relu_deriv(beta[i+1]))-c[i] + feedback_bias
            else:
                u[i]   = Y[i].mm(beta[i+1]*hard_deriv(beta[i+1], mean=hard_m, variance=hard_v))-c[i] + feedback_bias
                u_t[i] = Y[i].mm(beta_t[i+1]*hard_deriv(beta[i+1], mean=hard_m, variance=hard_v))-c[i] + feedback_bias
            ...
            delta_Z[i] = (min_Z - u[i]).mm(h[i].transpose(0, 1))
...
def train(folder_prefix=None, continuing_folder=None):
    ...
    if folder_prefix is not None:
        ...
        folder = "Tensorboard/" + "{} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} -".format(folder_prefix, str(ref), n_units_string, f_etas_string, b_etas_string, r_etas_string, W_std_string, Z_std_string, Y_std_string, output_burst_prob, min_Z, u_range, hard_m, hard_v) + info +"_+1"
    else:
        folder = None
