    ...
    for i in range(n_layers-1, 0, -1):
        if i == n_layers-1:
            ...
            if i == n_layers-2:
                ...
                u_t[i] = Y[i].mm(beta_t[i+1]*output_burst_prob*relu_deriv(beta[i+1]))
            else:
                u[i]   = Y[i].mm(beta[i+1]*hard_deriv(beta[i+1], mean=hard_m, variance=hard_v))
                u_t[i] = Y[i].mm(beta_t[i+1]*hard_deriv(beta[i+1], mean=hard_m, variance=hard_v))
