    ...
    for epoch_num in range(n_epochs):
        ...
        for example_num in range(n_examples):
            ...
            if (example_num+1) % store == 0:
                avg_costs           = [avg_costs[i]/(store*target_on_time) for i in range(n_layers-1)]
                avg_Y_costs         = [avg_Y_costs[i]/(store*target_on_time) for i in range(n_layers-2)]
                avg_Z_costs         = [avg_Z_costs[i]/(store*target_on_time) for i in range(n_layers-2)]
                avg_backprop_angles = [avg_backprop_angles[i]/(store*target_on_time) for i in range(n_layers-2)]
                min_us              = [min_us[i]/(store*target_on_time) for i in range(n_layers-2)]
                max_us              = [max_us[i]/(store*target_on_time) for i in range(n_layers-2)]
                min_hs              = [min_hs[i]/(store*target_on_time) for i in range(n_layers-1)]
                max_hs              = [max_hs[i]/(store*target_on_time) for i in range(n_layers-1)]
                avg_cs              = [avg_cs[i]/(store*target_on_time) for i in range(n_layers-2)]
                avg_std_cs          = [avg_std_cs[i]/(store*target_on_time) for i in range(n_layers-2)]
