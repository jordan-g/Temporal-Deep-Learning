    ...
    for i in range(n_layers-1, 0, -1):
        if i == n_layers-1:
            ...
            if i == n_layers-2:
                ...
                u[i]   = Y[i].mm(beta[i+1]*hard_deriv(beta[i+1], mean=hard_m, variancel=hard_vl, varianceh=hard_vh))/c[i]
                u_t[i] = Y[i].mm(beta_t[i+1]*hard_deriv(beta[i+1],mean=hard_m, variancel=hard_vl, varianceh=hard_vh))/c[i]
            ...
            e          = -(beta_t[i] - beta[i])*hard_deriv(beta[i], mean=hard_m, variancel=hard_vl, varianceh=hard_vh)
            delta_W[i] = e.mm(h[i-1].transpose(0, 1))
            delta_b[i] = e
...
def train(folder_prefix=None, continuing_folder=None):
    ...
    if folder_prefix is not None:
        ...
        folder = "Tensorboard/" + "{} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} - {} -".format(folder_prefix, str(ref), n_units_string, f_etas_string, b_etas_string, r_etas_string, W_std_string, Z_std_string, Y_std_string, output_burst_prob, min_Z, u_range, hard_m, hard_vl, hard_vh) + info +"_+1"
    else:
        folder = None
