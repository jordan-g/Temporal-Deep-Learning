    ...
    for i in range(n_layers-1, 0, -1):
        if i == n_layers-1:
            ...
            if i == n_layers-2:
                ...
                u[i]   = W[i+1].transpose(0, 1).mm(p[i+1]*h[i+1])/c
                u_t[i] = W[i+1].transpose(0, 1).mm(p_t[i+1]*h[i+1])/c
            ...
            delta_b_backprop[i] = W[i+1].transpose(0, 1).mm(delta_b_backprop[i+1])*h[i]
