...
def hard_deriv(x, mean, variancel, varianceh):
    y = torch.zeros_like(x)
    
    y[torch.le(x, mean+varianceh)*torch.ge(x, mean-variancel)] = 1
...
def forward_pass(W, b, v, h, f_input):
...
def forward(W, b, v, h, f_input, h_prev):
    h_prev[0] = f_input.unsqueeze(1)
...
def backward(Y, Z, W, b, u, p, beta, v, h, mean_c, c, t_input, beta_prev, h_prev):
    for i in range(n_layers-1, 0, -1):
        if i == n_layers-1:
            if t_input is not None:
                h[i] = 0.5*t_input.unsqueeze(1) + 0.5*h[i]

            beta[i] = output_burst_prob*h[i]

            cost[i]    = 0.5*torch.sum((beta[i] - beta_prev[i])**2)
            e          = -(beta[i] - beta_prev[i])*output_burst_prob*relu_deriv(h[i])
            delta_W[i] = e.mm(h_prev[i-1].transpose(0, 1))
            delta_b[i] = e

            delta_b_backprop[i] = -(beta[i] - beta_prev[i])*output_burst_prob*relu_deriv(h[i])
        else:
            c[i] = Z[i].mm(beta_prev[i])
            ...
            e          = -(beta[i] - beta_prev[i])*hard_deriv(beta_prev[i], mean=hard_m, variancel=hard_vl, varianceh=hard_vh)
            delta_W[i] = e.mm(h_prev[i-1].transpose(0, 1))
            delta_b[i] = e
            ...
            delta_Z[i] = (min_Z - u[i]).mm(beta_prev[i].transpose(0, 1))

    return cost, cost_Y, cost_Z, delta_W, delta_b, delta_Y, delta_Z, max_u, delta_b_backprop, mean_c, c


def update_weights(W, b, Y, Z, delta_W, delta_b, delta_Y, delta_Z, target_layers):
...
def train(folder_prefix=None, continuing_folder=None):
    ...
    W, b, Y, Z, v, h, u, p, beta, mean_c, c, h_prev, beta_prev = create_dynamic_variables(symmetric_weights=False)
    if continuing_folder is not None:
    ...
    for epoch_num in range(n_epochs):
        ...
        for example_num in range(n_examples):
            ...
            for timestep in range(time_per_example):
                forward(W, b, v, h, x, h_prev)

                # get closest target time to the current time -- target_onsets must be an array
                if timestep >= target_onsets[0]:
                    last_target_onset = target_onsets[target_onsets <= timestep].max()
                else:
                    last_target_onset = -1000
                time_since_target = timestep - last_target_onset
                target_on = time_since_target < target_on_time

                # get the layer that is currently receiving the target
                target_layers = []
                for i in range(1, n_layers):
                    if (n_layers-1-i) <= time_since_target < target_on_time+(n_layers-1-i):
                        target_layers.append(i)
                # print("Time {}, target layers: {}".format(timestep, target_layers))

                if target_on:
                    # print("Example {}, target on at time {}.".format(example_num, timestep))
                ...
                h_prev    = [0] + [ h[i].clone() for i in range(1, n_layers) ]
                beta_prev = [0] + [ beta[i].clone() for i in range(1, n_layers) ]
            ...
            if (example_num+1) % store == 0:
                avg_costs           = [avg_costs[i]/(store*time_per_example) for i in range(n_layers-1)]
                avg_Y_costs         = [avg_Y_costs[i]/(store*time_per_example) for i in range(n_layers-2)]
                avg_Z_costs         = [avg_Z_costs[i]/(store*time_per_example) for i in range(n_layers-2)]
                avg_backprop_angles = [avg_backprop_angles[i]/(store*time_per_example) for i in range(n_layers-2)]
                min_us              = [min_us[i]/(store*time_per_example) for i in range(n_layers-2)]
                max_us              = [max_us[i]/(store*time_per_example) for i in range(n_layers-2)]
                min_hs              = [min_hs[i]/(store*time_per_example) for i in range(n_layers-1)]
                max_hs              = [max_hs[i]/(store*time_per_example) for i in range(n_layers-1)]
                avg_cs              = [avg_cs[i]/(store*time_per_example) for i in range(n_layers-2)]
                avg_std_cs          = [avg_std_cs[i]/(store*time_per_example) for i in range(n_layers-2)]
                std_cs              = [torch.std(c[i]) for i in range(1, n_layers-1)]
                avg_W_std = [avg_W_std[i]/(store*time_per_example) for i in range(n_layers-1)]
                avg_W_mean = [avg_W_mean[i]/(store*time_per_example) for i in range(n_layers-1)]
                us = u[-2]
                ...
                if TensorB:
                    ...
                    writer.add_scalar('us_h_0', us[0], where)
                    writer.add_scalar('us_h_1', us[1], where)
...
def test(W, b):
    ...
    for i in range(n_test_examples):
        ...
        # do a forward pass
        forward_pass(W, b, v, h, f_input=x)
