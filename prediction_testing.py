...
def plot_outputs(folder, show=False):
    ...
    outputs_max = outputs[0, :].max(axis=0)
    outputs_min = outputs[0, :].min(axis=0)
    ...
    for i in range(outputs.shape[2]):
        ax.plot(np.arange(outputs.shape[-1]), np.mean(outputs[0, :, i], axis=0), colors[i], alpha=0.5)
        ax.plot(np.arange(targets.shape[-1]), np.mean(targets[0, :, i], axis=0), colors[i], linestyle='--')
    ...
    outputs_max = outputs[1, :].max(axis=0)
    outputs_min = outputs[1, :].min(axis=0)
    ...
    for i in range(outputs.shape[2]):
        ax.plot(np.arange(outputs.shape[-1]), np.mean(outputs[1, :, i], axis=0), colors[i], alpha=0.5)
        ax.plot(np.arange(targets.shape[-1]), np.mean(targets[1, :, i], axis=0), colors[i], linestyle='--')
