...
parser.add_argument("-y_std", help="Y std",type=lambda s: [float(item) for item in s.split(',')], default=[0, 0.1])
parser.add_argument("-f_etas", help="Forward Etas",type=lambda s: [float(item) for item in s.split(',')], default=[0, 0.5, 0.01])
parser.add_argument("-r_etas", help="Recurrent Etas",type=lambda s: [float(item) for item in s.split(',')], default=[0, 0.5])
...
if tmp == "Normal":
	...
	import network18 as net
	net.ref = 18
elif tmp == "network19":
	import network19 as net
	net.ref = 19
elif tmp == "network20":
